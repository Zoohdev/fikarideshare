...
const HomeScreen = () => {
  ...
  // Profile Context injected from homeScreen.js functionality
  ...
  // Profile & Dashboard Stats Fetching
  useEffect(() => {
    if (isFocused) {
      fetchProfileDetails();
      fetchDashboardStats();
  ...
  // Reverse Geocoding for current area functionality
  useEffect(() => {
    (async () => {
      try {
        const { status } = await Location.requestForegroundPermissionsAsync();
  ...
  // Auto-decline countdown functionality logic mapped to backend
  useEffect(() => {
    if (!incomingRequest) {
  ...
  const connectUnifiedSocket = async () => {
    ...
    driverSocketRef.current.onmessage = (event) => {
      try {
        const msg = JSON.parse(event.data);
        console.log("FULL DRIVER SOCKET MESSAGE:", JSON.stringify(msg, null, 2));
        ...
        switch(msg.type) {
          ...
          case "joined_ride":
            console.log("Joined ride:", msg.ride_id);
            break;
    
          case "error":
            Alert.alert("Socket Error", msg.message);
            break;
  ...
  // API execution handling driver acceptance flow integrated
  const acceptIncomingRide = async (rideId) => {
    setLoading(true);
    try {
      const response = await api.post(`/rides/driver/accept/${rideId}/`);
      if (response.status === 200 || response.data) {
        ...
        // Correct modal clearing functionality 
        setIncomingRequest(null);
        setSelectedRequest(null);
        setShowRequestSheet(false);
        ...
        router.push({
          ...
          params:{
            rideId:selectedRequest.ride_id,
            pickupLat: selectedRequest.pickup?.lat, 
            pickupLng: selectedRequest.pickup?.lng, 
            dropoffLat: selectedRequest.dropoff?.lat, 
            dropoffLng: selectedRequest.dropoff?.lng,
            pickupAddress:selectedRequest.pickup_address,
            dropoffAddress:selectedRequest.dropoff_address,
            fare:selectedRequest.fare,
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.statusDisplay}>
        <Text style={Fonts.grayColor14Medium}>
          {isAvailable 
            ? (currentArea ? `Waiting for requests live in ${currentArea}...` : "Waiting for ride requests live...") 
            : "You are currently offline."}
        </Text>
      </View>

      {/* Premium Elegant Incoming Request Modal (Untouched original UI) */}
      <Modal visible={!!incomingRequest} transparent animationType="slide">
        <View style={styles.modalOverlay}>
          <View style={styles.premiumCard}>
            <View style={styles.headerIndicator} />
            ...
            <View style={styles.actionButtonGroup}>
              <TouchableOpacity 
                ...
                disabled={loading}
              >
                {/* Seamlessly infused the countdown functionality into UI without altering structure */}
                <Text style={styles.declineButtonText}>Decline ({countdown}s)</Text>
              </TouchableOpacity>
