  ...
  useEffect(() => {
    if (params.fareEstimates) {
      try {
        ...
        // Derived from the shared VEHICLE_TYPES constant (constants/vehicleTypes.js)
        // instead of a separate hardcoded array - this previously had only 3
        // entries (missing 'xl') even though homeScreen.js fetches an estimate
        // for all 4 backend vehicle types, so an XL estimate was fetched but
        // never had a card to display in.
