...
export default function RiderInTripScreen() {
      ...
      wsRef.current.onmessage = (event) => {
        try {
          ...
          if (msgType === 'driver_location' || parsedMessage.latitude) {
            ...
            const lng = parsedMessage.longitude ?? data.longitude;
            if (lat && lng) {
              const newLocation = { latitude: Number(lat), longitude: Number(lng), heading: Number(parsedMessage.heading ?? data.heading) || 0 };
              setDriverLocation(newLocation);
              if (mapRef.current && hasFitInitially.current) {
                mapRef.current.animateCamera({ center: newLocation }, { duration: 1000 });
              }
            }
          }
          ...
          if (msgType === 'ride_status' || data.status) {
            const status = parsedMessage.status || data.status;
            // riderInTripScreen.js line 677 — change to:
            if (data.event === 'individual_dropped_off' && String(data.user_id) === String(uid)) {
              console.log("uid",uid);
              console.log("data.user_id",data.user_id)
              wsRef.current?.close();
              router.replace({ pathname: '/fareSummary/fareSummaryScreen', params: { rideId, fare: data.final_fare || rideData?.estimated_fare, role: 'rider' } });
              return;
            }
            if (status === 'completed') {
              wsRef.current?.close();
  ...
  const vehicleData = rideData?.vehicle || {};
  const driverData = rideData?.driver || {};
  const myParticipantCoords = rideData.participants?.find(
    (p) => String(p.user?.id) === String(userId)
  );
  
  const targetPickup = {
    latitude:  Number(pickupLat
      || myParticipantCoords?.pickup_location?.latitude
      || rideData.pickup_location?.latitude  || 0),
    longitude: Number(pickupLng
      || myParticipantCoords?.pickup_location?.longitude
      || rideData.pickup_location?.longitude || 0),
  };
  
  const targetDropoff = {
    latitude:  Number(dropoffLat
      || myParticipantCoords?.dropoff_location?.latitude
      || rideData.dropoff_location?.latitude  || 0),
    longitude: Number(dropoffLng
      || myParticipantCoords?.dropoff_location?.longitude
      || rideData.dropoff_location?.longitude || 0),
  };
  const originCoord = driverLocation || targetPickup;
  ...
  const myPickupAddress = myParticipant
    ? (myParticipant.pickup_address
        || myParticipant.pickup_location?.address
        || myParticipant.pickup_location?.formatted_address
        || rideData?.pickup_address
        || "Pickup unavailable")
    : (rideData?.pickup_address || "Pickup unavailable");
 
  const myDropoffAddress = myParticipant
    ? (myParticipant.dropoff_address
        || myParticipant.dropoff_location?.address
        || myParticipant.dropoff_location?.formatted_address
        || rideData?.dropoff_address
        || "Dropoff unavailable")
    : (rideData?.dropoff_address || "Dropoff unavailable");
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      <View style={styles.mapContainer}>
        ...
        >
          {/* Driver live vehicle marker (Flat Rotatable Image) */}
          {driverLocation && (
            <Marker
              coordinate={driverLocation}
              flat={true}
          ...
          {originCoord.latitude !== 0 && targetDropoff.latitude !== 0 && (
            <MapViewDirections
              ...
              onReady={(result) => {
                ...
                if (!hasFitInitially.current) {
                  hasFitInitially.current = true;
                  mapRef.current.fitToCoordinates(result.coordinates, { edgePadding: { top: 40, right: 40, bottom: 280, left: 40 }, animated: true });
                }
              }}
            />
          )}
        ...
        <ScrollView showsVerticalScrollIndicator={false}>
          ...
          <View style={styles.detailCard}>
            <View style={styles.addressBlockRow}>
              ...
              <View style={styles.addressTextFrame}>
                <Text style={styles.addressTitleLabel}>Pickup Point</Text>
                <Text style={styles.addressLineText}>{myPickupAddress}</Text>
              </View>
            </View>
            ...
            <View style={styles.addressBlockRow}>
              ...
              <View style={styles.addressTextFrame}>
                <Text style={styles.addressTitleLabel}>Dropoff Point</Text>
                <Text style={styles.addressLineText}>{myDropoffAddress}</Text>
              </View>
            </View>
          </View>
        </ScrollView>
...
const styles = StyleSheet.create({
  ...
  markerContainer: { alignItems: 'center', justifyContent: 'center' },
  markerTagFrame: { backgroundColor: '#0F172A', paddingHorizontal: 12, paddingVertical: 6, borderRadius: 8, borderWidth: 1, borderColor: '#334155', shadowColor: '#000', shadowOpacity: 0.2, shadowRadius: 3, elevation: 5, marginBottom: 4, alignItems: 'center', justifyContent: 'center' },
  markerTagText: { color: '#FFFFFF', fontSize: 12, fontWeight: '700', textAlign: 'center', flexWrap: 'wrap' },
  markerPinCircle: { width: 26, height: 26, borderRadius: 13, borderWidth: 3, borderColor: '#FFFFFF', alignItems: 'center', justifyContent: 'center', shadowColor: '#000', shadowOpacity: 0.3, shadowRadius: 4, elevation: 6 },
  markerPinNeedle: { width: 0, height: 0, backgroundColor: 'transparent', borderStyle: 'solid', borderLeftWidth: 5, borderRightWidth: 5, borderTopWidth: 6, borderLeftColor: 'transparent', borderRightColor: 'transparent', borderTopColor: '#FFFFFF', marginTop: -1 },
